...
export class UserService {
  ...
  getMyRepairs(propertyId: number): Observable<any> {
    const headers = this.createAuthorizationHeader();
    return this.http.get(`${this.baseUrl}/repairs?propertyId=${propertyId}`, {headers});
  }

  softDeleteUser(userId: any): Observable<any> {
    const headers = this.createAuthorizationHeader();
    return this.http.put(`${this.baseUrl}/users/${userId}/deactivate`, {headers});
  }
  ...
  putUpdateUser(userId: any, userData: any): Observable<any>{
    ...
    const headers = new HttpHeaders()
      .set('Content-Type', 'application/json');
    return this.http.put(url, JSON.stringify(userData), { headers })
      .pipe(
        retry(1),
